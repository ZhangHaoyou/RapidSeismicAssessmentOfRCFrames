...
def plot_beams(ax, z0, hs, nls, lls, nss, lss, bc, hb, bb, ts, color_beam):
    ...
    for ix in range(nls+1):
        ...
        for iy in range(nss):
            ly = iy*lss
            plot_opaque_cube(ax=ax, x=lx+(bc-bb)*0.5, y=ly+bc, z=z0+hs-hb, dx=bb, dy=lss-bc, dz=hb, color=color_beam)
    # plot the horizontal beams
    for iy in range(nss+1):
        ly = iy*lss
        for ix in range(nls):
            lx = ix*lls
            plot_opaque_cube(ax=ax, x=lx+bc, y=ly+(bc-bb)*0.5, z=z0+hs-hb, dx=lls-bc, dy=bb, dz=hb, color=color_beam)

def plot_slabs(ax, z0, hs, nls, lls, nss, lss, bc, bb, ts, color_slab):
    for ix in range(nls):
        lx = ix*lls
        for iy in range(nss):
            ly = iy*lss
            plot_opaque_cube(ax=ax, x=lx+bb, y=ly+bb, z=z0+hs-ts, dx=lls-bb, dy=lss-bb, dz=ts, color=color_slab)
...
def plot_story(ax, z0, hs, nls, lls, nss, lss, bc, hb, bb, ts, color_col, color_beam, color_slab):
    ...
    plot_columns(ax=ax, z0=z0, hs=hs, nls=nls, lls=lls, nss=nss, lss=lss, bc=bc, ts=ts, color_col=color_col)
    plot_slabs(ax=ax, z0=z0, hs=hs, nls=nls, lls=lls, nss=nss, lss=lss, bc=bc, bb=bb, ts=ts, color_slab=color_slab)
